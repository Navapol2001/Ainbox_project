...
class ChatAI:
    ...
    def __init__(self):
        ...
        self.eprompt = Ecommerce_page()
        self.iprompt = Information_page()
    ...
    def chat_activate(self, page_id: str, rank: str, history: Optional[List[dict]] = None) -> None:
        """
    ...
    def _chat_e_v1(self, messages: str, page_id: str, user_id: str) -> dict:
        ...
        ##################################################################################################   
            ...
            self.cal_con.consumption_log_msg(page_id=page_id,model=response['model'], prompt_input=response['usage']['prompt_tokens'] ,prompt_output=response['usage']['completion_tokens'] ,cost=cost)
            try:
                dict_answer = json.loads(self.clean_json_tag(dict_answer))
                break  # Exit the loop if JSON parsing is successful
            except :
                self.dict_chat[page_id].append({"role": "user", "content": f"# WARNING \n You Answer in Wrong Format \n Make Answer Following System Instruction"})
                # Continue the loop if there's an error
        ############################################ Change Chat Tier Functions Here ###################################################      
        #logging.info(dict_answer)
        llm_answer = {"answer":dict_answer['answer_response'],"customer_feeling":dict_answer['customer_feeling'],"topic":dict_answer['topic'],"answer_able":str(dict_answer['answer_able'])}
        return self.clean_msg(llm_answer)

    def prompt_activate(self, page_id: str, rank: str) -> str:
        """
        ...
        if str(rank) in ["EC1","EC2"]:
            return self.eprompt.generate_admin_prompt(page_id, rank)
        elif str(rank) in ["IP1","IP2"]:
            return self.iprompt.generate_admin_prompt(page_id, rank)
    ...
    def clean_msg(self, msg: str) -> str:
        ...
        """
        cleaned_msg = {key: value.replace("**", "").replace("##", "#").replace("*", "•").replace("\n\n", "\n").replace("  ", " ").strip() for key, value in msg.items()}
        #cleaned_msg = re.sub(r'[^\x00-\x7F]+', '', cleaned_msg)

        return cleaned_msg
    
    def clean_json_tag(self, text: str) -> str:
        """
